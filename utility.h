...
void Show(cv::Mat& img, string title = "Display Window"){
...
void ShowHorizontal(vector<cv::Mat>& imgs, string title = "Display Window"){
    cv::Mat target = imgs[0];
    ...
    // cv::namedWindow( title, cv::WINDOW_AUTOSIZE);// Create a window for display.
    // cv::imshow(title, img2);                   // Show our image inside it.
    // cv::waitKey(0);
    // cv::destroyAllWindows();
